...
namespace MecsTimeKeeper.Services
{
    class BlocklistService
    {
        ...
        public bool CheckIsEmployeeIfBlock(int emp_id)
        {
            var sql = string.Format("SELECT id FROM {0} WHERE emp_id = @emp_id", table_name);
            var result = this._conn.Query<int>(sql, new { emp_id = emp_id }).SingleOrDefault();
            if(result > 0)
            {
                return true;
            }
