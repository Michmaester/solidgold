<template>
	<div>
		...
		<div class="has-background-pageheader px-4 py-2 w-full flex items-center justify-between border-b border-gray-200">
			...
			<h3 class="has-text-header">Stock-Outs</h3>
			<div>
		...
		<div class="grid grid-cols-8 gap-4 p-4 has-background-pageheader">
			...
			<div class="col-span-8 wrapper-h bg-white">
				...
				<div class="text-sm p-2">
					...
					<b-table :striped="true" :narrowed="true" :hoverable="true" :loading="loading" paginated backend-pagination :pagination-rounded="true" pagination-size="is-small" :total="totalDataCount" :per-page="pageSize" @page-change="onPageChange" backend-sorting :default-sort-direction="defaultSortOrder" :default-sort="[sortField, sortOrder]" @sort="onSort" sort-icon="arrow-up" sort-icon-size="is-small" :pagination-simple="false" :pagination-position="'top'" :selected.sync="selected_stockouts" @select="SelectedStockOuts" :data="stockOuts">
						...
						<b-table-column v-slot="props" label="Date Out">
							{{ props.row.dateStockout }}
						</b-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			SelectedStockOuts(data) {
				this.selected_stockouts = data
			},
			...
			async LoadAsyncData() {
				...
				const stocks = await this.GetStockOuts(params)

				this.stockOuts = []
				this.stockOuts = stocks.data.results
