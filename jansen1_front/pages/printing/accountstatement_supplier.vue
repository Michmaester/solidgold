	...
	<div class="flex flex-col">
		...
		<div v-if="printdocData" class="p-4">
			...
			<div class="mt-4 leading-none text-xs">
				<p><span class="font-bold">Supplier :</span> {{ printdocData.docMeta.supplier.name }}</p>
				<p><span class="font-bold">Address :</span> {{ printdocData.docMeta.supplier.address }}</p>
			</div>
					...
					<tbody>
						<tr v-for="(item,idx) in printdocData.result" :key="idx">
							...
							<td v-for="(field,i) in printdocData.docMeta.fields" :key="i">
								<span v-if="field.isAmount">{{ $formatAmount(GetObjectPropData(item,field.field),2) }}</span>
			...
			<div class="mt-6 flex justify-end">
				<div class="py-2 px-6 border border-gray-400 text-center">
					<p>Total Balance</p>
					<span class="block font-bold text-xl">{{ $formatAmountCurrency(printdocData.docMeta.otherDatas.pos_balance_totals) }}</span>
				</div>
			</div>
